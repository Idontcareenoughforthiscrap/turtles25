...
-- Check if we can drop down (no block below)
function canDropDown()
...
function solveMaze()
    ...
    while steps < maxSteps do
        ...
        -- ABSOLUTE HIGHEST PRIORITY: Try turtle.down() command before anything else
        print("Checking if turtle can move down...")
        local blockBelow = turtle.detectDown()
        print("Block below detected: " .. tostring(blockBelow))
        
        if not blockBelow then
            print("No block below - attempting to move down...")
            if turtle.down() then
                print("SUCCESS! Moved down one block - ABSOLUTE HIGHEST PRIORITY (step " .. steps .. ")")
                -- Continue to next iteration to check for yellow concrete below
                sleep(0.5)
                goto continue
            else
                print("FAILED to move down despite no block detected!")
            end
        else
            print("Cannot move down - block detected below")
        end
        ...
        -- Additional check: Try to move down again if we haven't moved yet this step
        if not blockBelow and turtle.down() then
            print("SUCCESS! Second attempt to move down worked (step " .. steps .. ")")
            sleep(0.5)
            goto continue
        end
            ...
            if isOverVisited() then
                ...
                if bestDirection then
                    ...
                    if bestDirection.name == "forward" then
                        -- Check if we can drop down before moving forward
                        if turtle.down() then
                            print("Dropped down before moving to least visited block forward!")
                            goto continue
                        end
                        moveForward()
                        updatePosition()
                        -- Check if we can drop down after moving to new position
                        if turtle.down() then
                            print("Dropped down after moving to least visited block forward!")
                            goto continue
                        end
                        turnCounter = 0
                        print("Moved to least visited block forward (step " .. steps .. ")")
                    elseif bestDirection.name == "right" then
                        -- Check if we can drop down before turning right
                        if turtle.down() then
                            print("Dropped down before moving to least visited block right!")
                            goto continue
                        end
                        turnRight()
                        updateDirection("right")
                        -- Check if we can drop down before moving forward
                        if turtle.down() then
                            print("Dropped down after turning right!")
                            goto continue
                        end
                        moveForward()
                        updatePosition()
                        -- Check if we can drop down after moving to new position
                        if turtle.down() then
                            print("Dropped down after moving to least visited block right!")
                            goto continue
                        end
                        turnCounter = 0
                        print("Moved to least visited block right (step " .. steps .. ")")
                    elseif bestDirection.name == "left" then
                        -- Check if we can drop down before turning left
                        if turtle.down() then
                            print("Dropped down before moving to least visited block left!")
                            goto continue
                        end
                        turnLeft()
                        updateDirection("left")
                        -- Check if we can drop down before moving forward
                        if turtle.down() then
                            print("Dropped down after turning left!")
                            goto continue
                        end
                        moveForward()
                        updatePosition()
                        -- Check if we can drop down after moving to new position
                        if turtle.down() then
                            print("Dropped down after moving to least visited block left!")
                            goto continue
                        end
                        turnCounter = 0
                ...
                local canGoDown = canDropDown()
                
                print("Step " .. steps .. " - Left=" .. tostring(canGoLeft) .. ", Right=" .. tostring(canGoRight) .. ", Forward=" .. tostring(canGoForward) .. ", Down=" .. tostring(canGoDown) .. ", TurnCount=" .. turnCounter)
                
                -- If we've made too many turns, force forward movement
                if turnCounter >= maxTurns and canGoForward then
                    -- Check if we can drop down before forced forward movement
                    if turtle.down() then
                        print("Dropped down before forced forward movement!")
                        goto continue
                    end
                    moveForward()
                    updatePosition()
                    -- Check if we can drop down after forced forward movement
                    if turtle.down() then
                        print("Dropped down after forced forward movement!")
                        goto continue
                    end
                ...
                elseif canGoLeft and turnCounter < maxTurns then
                    -- Check if we can drop down before turning left
                    if turtle.down() then
                        print("Dropped down before turning left!")
                        goto continue
                    end
                    ...
                    -- Check if we can drop down after moving to new position
                    if turtle.down() then
                        print("Dropped down after turning left and moving!")
                        goto continue
                    end
                ...
                elseif canGoRight and turnCounter < maxTurns then
                    -- Check if we can drop down before turning right
                    if turtle.down() then
                        print("Dropped down before turning right!")
                        goto continue
                    end
                    ...
                    -- Check if we can drop down after moving to new position
                    if turtle.down() then
                        print("Dropped down after turning right and moving!")
                        goto continue
                    end
                    turnCounter = turnCounter + 1
                ...
                elseif canGoForward then
                    -- Check if we can drop down before moving forward
                    if turtle.down() then
                        print("Dropped down before moving forward!")
                        goto continue
                    end
                    ...
                    -- Check if we can drop down after moving to new position
                    if turtle.down() then
                        print("Dropped down after moving forward!")
                        goto continue
                    end
                    turnCounter = 0 -- Reset turn counter after moving forward
                    print("Moved forward (step " .. steps .. ")")
                else
                    -- Check if we can drop down before turning around
                    if turtle.down() then
                        print("Dropped down before turning around!")
                        goto continue
                    end
